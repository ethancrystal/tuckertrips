-- Complete Tucker Trips Database Schema

-- Drop existing tables (this will also drop triggers and policies)
...
CREATE POLICY "Users can view friends trips" ON public.trips FOR SELECT USING (
...
CREATE POLICY "Users can view categories of accessible trips" ON public.trip_categories FOR SELECT USING (
...
CREATE POLICY "Users can create categories for own trips" ON public.trip_categories FOR INSERT WITH CHECK (
  trip_id IN (SELECT id FROM public.trips WHERE user_id = auth.uid())
);
CREATE POLICY "Users can update categories for own trips" ON public.trip_categories FOR UPDATE USING (
  trip_id IN (SELECT id FROM public.trips WHERE user_id = auth.uid())
);
CREATE POLICY "Users can delete categories for own trips" ON public.trip_categories FOR DELETE USING (
  trip_id IN (SELECT id FROM public.trips WHERE user_id = auth.uid())
);
