...
export async function checkTripAccess(supabase, tripId, userId) {
  ...
  // Rule 2.5: Friends-visibility trips are readable by accepted friends
  // (either direction of the friendship row, status must be 'accepted').
  if (userId && trip.visibility === 'friends') {
    const { data: friendship } = await supabase
      .from('friendships')
      .select('id')
      .eq('status', 'accepted')
      .or(
        `and(user_id.eq.${userId},friend_id.eq.${trip.user_id}),` +
          `and(user_id.eq.${trip.user_id},friend_id.eq.${userId})`
      )
      .limit(1)
      .maybeSingle()
